...
export function PurchaseOrderForm({ open, suppliers, onClose, onSuccess }) {
    ...
    const onSubmit = async (data) => {
        if (items.length === 0)
            return;
        const result = await createPurchaseOrder(data.supplier_id, items.map(i => ({ productId: i.productId, quantity: i.quantity, unitCost: i.unitCost })), data.expected_delivery_date || undefined, data.notes || undefined);
        if (result) {
            form.reset();
            setItems([]);
            onSuccess();
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="supplier_id" render={({ field }) => (<FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {suppliers.filter(s => s.is_active).map((supplier) => (<SelectItem key={supplier.id} value={supplier.id}>
                            {supplier.name}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
            ...
            <FormField control={form.control} name="expected_delivery_date" render={({ field }) => (<FormItem>
            ...
            <div className="border rounded-lg p-4 space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <Input type="number" placeholder="Qty" className="w-20" value={quantity} onChange={(e) => setQuantity(e.target.value)} min="1"/>
                <Input type="number" placeholder="Cost" className="w-24" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} min="0" step="0.01"/>
